...
class Camera_t{
    public:
        Camera_t(TransformMatrix_t* transformation, std::string filename, Vec3f up, const double (&fov)[2], const unsigned int (&subpix)[2], std::list<Medium_t*> &medium_list, Skybox_t* skybox, unsigned int max_bounces, double gammaind);
        Camera_t(){};
        ...
        virtual void accumulateWrite(Scene_t* scene, unsigned int n_iter = 1000000000, unsigned int interval = 1);
