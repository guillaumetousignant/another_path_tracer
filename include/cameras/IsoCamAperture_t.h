...
namespace APTracer { namespace Cameras {
    ...
    class IsoCamAperture_t final : public Camera_t {
        public:
             ...
             */
            IsoCamAperture_t(TransformMatrix_t* transformation, const std::string &filename, APTracer::Entities::Vec3f up, std::array<double, 2> fov, std::array<unsigned int, 2> subpix, ImgBuffer_t* image, std::list<Medium_t*> medium_list, Skybox_t* skybox, unsigned int max_bounces, double focus_distance, double aperture, double gammaind);
