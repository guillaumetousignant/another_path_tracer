#ifndef APTRACER_RECCAMMOTIONBLURAPERTURE_T_H
#define APTRACER_RECCAMMOTIONBLURAPERTURE_T_H
...
namespace APTracer { namespace Cameras {
    ...
    /**
     * @brief The rec cam motionblur aperture class describes a camera that uses a rectangular projection, a shutter for motion blur, and has an aperture for depth of field.
     * 
    ...
    class RecCamMotionblurAperture_t  final: public Camera_t{
        public:
            /**
             * @brief Construct a new RecCamMotionblurAperture_t object. Most arguments are passed to the Camera_t constructor.
             * 
             ...
             */
            RecCamMotionblurAperture_t(TransformMatrix_t* transformation, const std::string &filename, Vec3f up, std::array<double, 2> fov, std::array<unsigned int, 2> subpix, ImgBuffer_t* image, std::list<Medium_t*> medium_list, Skybox_t* skybox, unsigned int max_bounces, double focal_length, double aperture, std::array<double, 2> time, double gammaind);
            
            /**
             * @brief Destroy the RecCamMotionblurAperture_t object. Does nothing.
             */
            virtual ~RecCamMotionblurAperture_t() final = default;
