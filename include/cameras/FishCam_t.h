#ifndef APTRACER_CAMERAS_FISHCAM_T_H
#define APTRACER_CAMERAS_FISHCAM_T_H
...
namespace APTracer { namespace Cameras {
    ...
    /**
     * @brief The fish cam class describes a fisheye camera that uses an equidistant projection.
     * 
     ...
     */
    class FishCam_t final : public Camera_t {
        public:
            /**
             * @brief Construct a new FishCam_t object. Most arguments are passed to the Camera_t constructor.
             * 
             ...
             */
            FishCam_t(TransformMatrix_t* transformation, const std::string &filename, Vec3f up, std::array<double, 2> fov, std::array<unsigned int, 2> subpix, ImgBuffer_t* image, std::list<Medium_t*> medium_list, Skybox_t* skybox, unsigned int max_bounces, double gammaind);
