#ifndef APTRACER_ISOCAMMOTIONBLURAPERTURE_T_H
#define APTRACER_ISOCAMMOTIONBLURAPERTURE_T_H
...
namespace APTracer { namespace Cameras {
    ...
    /**
     * @brief The iso cam motionblur aperture class describes a camera that uses an isometric projection, a shutter for motion blur, and has an aperture for depth of field.
     * 
     ...
     */
    class IsoCamMotionblurAperture_t  final: public Camera_t{
        public:
            /**
             * @brief Construct a new IsoCamMotionblurAperture_t object. Most arguments are passed to the Camera_t constructor.
             * 
             ...
             */
            IsoCamMotionblurAperture_t(TransformMatrix_t* transformation, const std::string &filename, Vec3f up, const double (&fov)[2], const unsigned int (&subpix)[2], ImgBuffer_t* image, std::list<Medium_t*> medium_list, Skybox_t* skybox, unsigned int max_bounces, double focal_length, double aperture, double (&time)[2], double gammaind);
            
            /**
             * @brief Destroy the IsoCamMotionblurAperture_t object. Does nothing.
             */
            virtual ~IsoCamMotionblurAperture_t() final;
