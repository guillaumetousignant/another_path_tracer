...
namespace APTracer { namespace Acceleration {
    ...
    class AccelerationMultiGridVector_t final : public AccelerationStructure_t{
        public:
             ...
             */
            AccelerationMultiGridVector_t(Shape_t** items, size_t n_items, size_t min_res, size_t max_res, size_t max_cell_content, unsigned int max_grid_level);
             ...
             * @param max_grid_level Maximum recursion level of the grid. 0 is no subgrids, 1 means cells from the top-level grid can be single level grids, etc.
             */
            AccelerationMultiGridVector_t(Shape_t** items, size_t n_items, std::array<Vec3f, 2> coordinates, unsigned int level, size_t min_res, size_t max_res, size_t max_cell_content, unsigned int max_grid_level);
            
            /**
             * @brief Destroy the AccelerationMultiGridVector_t object, destroying the cells and grids it owns.
             */
            virtual ~AccelerationMultiGridVector_t() final = default;

            std::vector<std::unique_ptr<AccelerationStructure_t>> cells_; /**< @brief Array of all the cells and grids contained in the acceleration structure. Cells use vectors of shapes, for fast iteraring and low memory use.*/
