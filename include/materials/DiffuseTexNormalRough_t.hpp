#ifndef APTRACER_MATERIALS_DIFFUSETEXNORMALROUGH_T_HPP
#define APTRACER_MATERIALS_DIFFUSETEXNORMALROUGH_T_HPP
...
namespace APTracer { namespace Materials {
    ...
    /**
     * @brief The diffuse tex normal rough class describes a material that reflects rays in random directions around the surface normal taken from a normal map, to model diffuse reflection. Its
     * reflective colour is taken from a texture. Its roughness is taken from a texture.
     *
     ...
     * This material represents non uniformly finely rough surfaces, such as wood floors.
     */
    class DiffuseTexNormalRough_t final : public Entities::Material_t {
        public:
            /**
             * @brief Construct a new DiffuseTexNormalRough_t object with an emissive colour, a reflective texture, a roughness, and a normal map.
             *
             ...
             * @param roughness Texture containing the attenuation of the colour as the angle between the incident ray and the surface normal increase. 0 to 1, 0 being an ideal diffusely reflective
             * surface, obeying Lambert's cosine law and 1 being very rough (no attenuation).
             */
            DiffuseTexNormalRough_t(const Entities::Vec3f& emission, const Entities::Texture_t* texture, const Entities::Texture_t* normal_map, const Entities::Texture_t* roughness);
            ...
            const Entities::Texture_t* roughness_; /**< @brief Texture containing the attenuation of the colour as the angle between the incident ray and the surface normal increase. 0 to 1, 0 being
                                  an ideal diffusely reflective surface, obeying Lambert's cosine law and 1 being very rough (no attenuation).*/
