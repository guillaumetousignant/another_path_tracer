...
namespace APTracer { namespace Entities {
    ...
    class Medium_t {
        public:
             ...
             * The function can also scatter rays. This means that the ray's direction and origin are changed by the medium, for example to model intersection with a particle. In this case, the function
             * must return true, to avoid resolving the ray-shape intersection, as the ray has changed direction and possibly doesn't hit the shape anymore. It is called after a hit has been found, but
             * before bouncing on the shape's material. This bounce is not performed if the ray is scattered, and the ray's distance should be changed accordingly.
             * 
