...
IsoCamMotionblur_t::IsoCamMotionblur_t(TransformMatrix_t* transformation, std::string filename, Vec3f up, const double (&fov)[2], const unsigned int (&subpix)[2], ImgBuffer_t* image, std::list<Medium_t*> &medium_list, Skybox_t* skybox, unsigned int max_bounces, double (&time)[2], double gammaind) 
        ...
        for (unsigned int i = 0; i < image_->size_x_; i++){
            ...
            for (unsigned int k = 0; k < subpix_[0]; k++){
                for (unsigned int l = 0; l < subpix_[1]; l++){
                    ...
                    Vec3f ray_origin = -vertical_int * pixel_span_y * ((double)j - (double)image_->size_y_/2.0 + 0.5) + horizontal_int * pixel_span_x * ((double)i - (double)image_->size_x_/2.0 + 0.5);
