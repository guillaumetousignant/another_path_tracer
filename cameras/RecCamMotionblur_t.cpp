...
RecCamMotionblur_t::RecCamMotionblur_t(TransformMatrix_t* transformation, std::string filename, Vec3f up, const double (&fov)[2], const unsigned int (&subpix)[2], ImgBuffer_t* image, std::list<Medium_t*> &medium_list, Skybox_t* skybox, unsigned int max_bounces, double (&time)[2], double gammaind) 
...
void RecCamMotionblur_t::raytrace(const Scene_t* scene) {
    ...
    focus_point_last = origin_last_ + direction_last_;
    tot_subpix = subpix_[0]*subpix_[1];
    pixel_span_y = std::tan(fov_[0]/2.0) * 2.0/image_->size_y_;
    pixel_span_x = std::tan(fov_[1]/2.0) * 2.0/image_->size_x_;
        ...
        for (unsigned int i = 0; i < image_->size_x_; i++){
            ...
            for (unsigned int k = 0; k < subpix_[0]; k++){
                for (unsigned int l = 0; l < subpix_[1]; l++){
                    ...
                    Vec3f ray_vec = focus_point_int - vertical_int * (pixel_span_y * ((double)j - (double)image_->size_y_/2.0 + 0.5) + subpix_span_y * ((double)k - (double)subpix_[0]/2.0 + jitter_y))
                                 + horizontal_int * (pixel_span_x * ((double)i - (double)image_->size_x_/2.0 + 0.5) + subpix_span_x * ((double)l - (double)subpix_[1]/2.0 + jitter_x))
                                 - origin_int;

                    Ray_t ray = Ray_t(origin_int, ray_vec.normalize()+Vec3f(0.00001, 0.0, 0.0), Vec3f(), Vec3f(1.0, 1.0, 1.0), medium_list_, rand_time);
