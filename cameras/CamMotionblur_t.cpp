...
CamMotionblur_t::CamMotionblur_t(TransformMatrix_t* transformation, std::string filename, Vec3f up, const double (&fov)[2], const unsigned int (&subpix)[2], ImgBuffer_t* image, std::list<Medium_t*> &medium_list, Skybox_t* skybox, unsigned int max_bounces, double (&time)[2], double gammaind) 
        ...
        for (unsigned int i = 0; i < image_->size_x_; i++){
            ...
            for (unsigned int k = 0; k < subpix_[0]; k++){
                for (unsigned int l = 0; l < subpix_[1]; l++){
                    ...
                    pix_vec = to_xyz_offset(pix_vec, direction_int, horizontal_int, vertical_int);

                    Ray_t ray = Ray_t(origin_int, pix_vec, Vec3f(), Vec3f(1.0, 1.0, 1.0), medium_list_, rand_time);
