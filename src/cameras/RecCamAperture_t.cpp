...
RecCamAperture_t::RecCamAperture_t(TransformMatrix_t* transformation,
...
auto RecCamAperture_t::raytrace(const Scene_t* scene) -> void {
    ...
    for (size_t index = 0; index < image_->size_y_ * image_->size_x_; ++index) {
        ...
        const Vec3f pix_vec = focus_point - pixel_span_y * (static_cast<double>(j) - static_cast<double>(image_->size_y_) / 2.0 + 0.5)
                            + pixel_span_x * (static_cast<double>(i) - static_cast<double>(image_->size_x_) / 2.0 + 0.5);
        ...
        for (unsigned int subindex = 0; subindex < subpix_[0] * subpix_[1]; ++subindex) {
            ...
            const Vec3f subpix_vec = (pix_vec - subpix_span_y * (static_cast<double>(k) - static_cast<double>(subpix_[0]) / 2.0 + jitter_y)
                                      + subpix_span_x * (static_cast<double>(l) - static_cast<double>(subpix_[1]) / 2.0 + jitter_x) - origin2)
                                         .normalize_inplace();
...
auto RecCamAperture_t::autoFocus(const Scene_t* scene, std::array<double, 2> position) -> void {
    ...
    const Vec3f ray_vec    = (direction_ - span_y * vertical * (position[1] - 0.5) + span_x * horizontal * (position[0] - 0.5)).normalize_inplace();
