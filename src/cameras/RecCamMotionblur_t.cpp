...
RecCamMotionblur_t::RecCamMotionblur_t(TransformMatrix_t* transformation, const std::string &filename, Vec3f up, std::array<double, 2> fov, std::array<unsigned int, 2> subpix, ImgBuffer_t* image, std::list<Medium_t*> medium_list, Skybox_t* skybox, unsigned int max_bounces, std::array<double, 2> time, double gammaind) 
...
auto RecCamMotionblur_t::raytrace(const Scene_t* scene) -> void {
    ...
    const double pixel_span_y_last = std::tan(fov_last_[0]/2.0) * 2.0/image_->size_y_;
    const double pixel_span_x_last = std::tan(fov_last_[1]/2.0) * 2.0/image_->size_x_;
