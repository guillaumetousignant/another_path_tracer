...
RecCamMotionblurAperture_t::RecCamMotionblurAperture_t(TransformMatrix_t* transformation,
...
auto RecCamMotionblurAperture_t::raytrace(const Scene_t* scene) -> void {
    ...
    for (size_t index = 0; index < image_->size_y_ * image_->size_x_; ++index) {
        ...
        for (unsigned int subindex = 0; subindex < subpix_[0] * subpix_[1]; ++subindex) {
            ...
            const Vec3f origin_int         = origin_ * rand_time + origin_last_ * (1.0 - rand_time) + vertical_int * std::cos(rand_theta) * rand_r + horizontal_int * std::sin(rand_theta) * rand_r;
