...
IsoCamMotionblur_t::IsoCamMotionblur_t(TransformMatrix_t* transformation,
...
auto IsoCamMotionblur_t::raytrace(const Scene_t* scene) -> void {
    ...
    for (size_t index = 0; index < image_->size_y_ * image_->size_x_; ++index) {
        ...
        for (unsigned int subindex = 0; subindex < subpix_[0] * subpix_[1]; ++subindex) {
            ...
            const Vec3f ray_origin = -vertical_int * pixel_span_y_int * (static_cast<double>(j) - static_cast<double>(image_->size_y_) / 2.0 + 0.5)
                                   + horizontal_int * pixel_span_x_int * (static_cast<double>(i) - static_cast<double>(image_->size_x_) / 2.0 + 0.5) + origin_int
                                   - vertical_int * subpix_span_y_int * (static_cast<double>(k) - static_cast<double>(subpix_[0]) / 2.0 + jitter_y)
