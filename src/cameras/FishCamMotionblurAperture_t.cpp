...
using APTracer::Cameras::FishCamMotionblurAperture_t;
...
FishCamMotionblurAperture_t::FishCamMotionblurAperture_t(TransformMatrix_t* transformation,
...
auto FishCamMotionblurAperture_t::raytrace(const Scene_t* scene) -> void {
...
auto FishCamMotionblurAperture_t::autoFocus(const Scene_t* scene, std::array<double, 2> position) -> void {
    ...
    const std::array<double, 2> pix_position = {(position[0] - 0.5) * fov_[1], -(position[1] - 0.5) * fov_[0]};
...
auto FishCamMotionblurAperture_t::zoom(double factor) -> void {
    fov_buffer_ = {fov_[0] * factor, fov_[1] * factor};
}

auto FishCamMotionblurAperture_t::write(const std::string& file_name) -> void {
    image_->write(file_name, gammaind_);
}

auto FishCamMotionblurAperture_t::write() -> void {
    image_->write(filename_, gammaind_);
}

auto FishCamMotionblurAperture_t::show() const -> void {
    // What to do here?
}

auto FishCamMotionblurAperture_t::reset() -> void {
    image_->reset();
}