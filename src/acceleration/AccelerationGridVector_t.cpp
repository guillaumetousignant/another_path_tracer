...
AccelerationGridVector_t::AccelerationGridVector_t(const std::vector<Shape_t*>& items, size_t min_res, size_t max_res) : level_(0), min_res_(min_res), max_res_(max_res), size_(items.size()) {
    ...
    cells_     = std::vector<std::unique_ptr<GridCellVector_t>>(cell_res_[0] * cell_res_[1] * cell_res_[2]);
...
AccelerationGridVector_t::AccelerationGridVector_t(const std::vector<Shape_t*>& items, std::array<Vec3f, 2> coordinates, unsigned int level, size_t min_res, size_t max_res) :
...
auto AccelerationGridVector_t::add(Shape_t* item) -> void {
    Vec3f min1 = Vec3f(std::numeric_limits<double>::max());
    Vec3f max1 = Vec3f(std::numeric_limits<double>::lowest());
...
auto AccelerationGridVector_t::move(Shape_t* item) -> void {}
