            ...
            for (auto y = static_cast<size_t>(min1[1]); y <= static_cast<size_t>(max1[1]); ++y) {
                for (auto x = static_cast<size_t>(min1[0]); x <= static_cast<size_t>(max1[0]); ++x) {
                    if (!temp_cells[x + y * cell_res_[0] + z * cell_res_[0] * cell_res_[1]]) {
                        temp_cells[x + y * cell_res_[0] + z * cell_res_[0] * cell_res_[1]] = std::make_unique<GridCell_t>();
                    }
    ...
    for (size_t i = 0; i < (cell_res_[0] * cell_res_[1] * cell_res_[2]); ++i) {
        if (temp_cells[i]) {
            if ((temp_cells[i]->size() > max_cell_content_) && (level_ < max_grid_level_)) {
                std::vector<Shape_t*> temp_elements;
                temp_elements.reserve(temp_cells[i]->items_.size());
                std::copy(std::begin(temp_cells[i]->items_), std::end(temp_cells[i]->items_), std::back_inserter(temp_elements));
                ...
                cells_[i] = std::make_unique<AccelerationMultiGrid_t>(temp_elements, cell_extent, level_ + 1, min_res_, max_res_, max_cell_content_, max_grid_level_);
            }
            else {
...
AccelerationMultiGrid_t::AccelerationMultiGrid_t(const AccelerationMultiGrid_t& other) :
...
auto AccelerationMultiGrid_t::operator=(const AccelerationMultiGrid_t& other) -> AccelerationMultiGrid_t& {
    if (this != &other) {
        *this = AccelerationMultiGrid_t(other);
    }
    return *this;
}
...
auto AccelerationMultiGrid_t::add(Shape_t* item) -> void {
    Vec3f min1 = Vec3f(std::numeric_limits<double>::max());
    Vec3f max1 = Vec3f(std::numeric_limits<double>::lowest());
...
auto AccelerationMultiGrid_t::remove(Shape_t* const item) -> void {
    Vec3f min1 = Vec3f(std::numeric_limits<double>::max());
    Vec3f max1 = Vec3f(std::numeric_limits<double>::lowest());
...
auto AccelerationMultiGrid_t::move(Shape_t* item) -> void {}

auto AccelerationMultiGrid_t::size() const -> size_t {
    return size_;
}

auto AccelerationMultiGrid_t::clone() const -> std::unique_ptr<AccelerationStructure_t> {
    return std::make_unique<AccelerationMultiGrid_t>(*this);
}
