...
AccelerationMultiGridVector_t::AccelerationMultiGridVector_t(Shape_t** items, size_t n_items, unsigned int min_res, unsigned int max_res, unsigned int max_cell_content, unsigned int max_grid_level) : 
    ...
    for (unsigned int i = 0; i < (cell_res_[0]*cell_res_[1]*cell_res_[2]); i++) {
        if (temp_cells[i] != nullptr) {
            if ((temp_cells[i]->n_obj_ > max_cell_content_) && (level_ < max_grid_level_)) {
                ...
                cells_[i] = new AccelerationMultiGridVector_t(temp_cells[i]->items_.data(), temp_cells[i]->n_obj_, cell_extent, level_+1, min_res_, max_res_, max_cell_content_, max_grid_level_);
...
AccelerationMultiGridVector_t::AccelerationMultiGridVector_t(Shape_t** items, size_t n_items, std::array<Vec3f, 2> coordinates, unsigned int level, unsigned int min_res, unsigned int max_res, unsigned int max_cell_content, unsigned int max_grid_level) : 
