    ...
    for (size_t i = 0; i < (cell_res_[0]*cell_res_[1]*cell_res_[2]); ++i) {
        if (temp_cells[i]) {
            if ((temp_cells[i]->size() > max_cell_content_) && (level_ < max_grid_level_)) {
                ...
                cells_[i] = std::make_unique<AccelerationMultiGridVector_t>(temp_cells[i]->items_, cell_extent, level_+1, min_res_, max_res_, max_cell_content_, max_grid_level_);
            }
            else {
...
AccelerationMultiGridVector_t::AccelerationMultiGridVector_t(const std::vector<Shape_t*>& items, std::array<Vec3f, 2> coordinates, unsigned int level, size_t min_res, size_t max_res, size_t max_cell_content, unsigned int max_grid_level) : 
...
auto AccelerationMultiGridVector_t::move(Shape_t* item) -> void {
