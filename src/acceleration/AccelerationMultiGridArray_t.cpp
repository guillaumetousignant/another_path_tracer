    ...
    for (size_t i = 0; i < (cell_res_[0]*cell_res_[1]*cell_res_[2]); ++i) {
        if (temp_cells[i]) {
            if ((temp_cells[i]->n_obj_ > max_cell_content_) && (level_ < max_grid_level_)) {
                ...
                cells_[i] = std::make_unique<AccelerationMultiGridArray_t>(temp_cells[i]->items_, temp_cells[i]->n_obj_, cell_extent, level_+1, min_res_, max_res_, max_cell_content_, max_grid_level_);
            }
            else {
