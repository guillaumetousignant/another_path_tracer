...
APTracer::Materials::DiffuseTexNormalRough_t::DiffuseTexNormalRough_t(const Entities::Vec3f& emission,
                                                                      const APTracer::Entities::Texture_t* texture,
                                                                      const APTracer::Entities::Texture_t* normal_map,
                                                                      const APTracer::Entities::Texture_t* roughness) :
...
auto APTracer::Materials::DiffuseTexNormalRough_t::bounce(std::array<double, 2> uv, const APTracer::Entities::Shape_t* hit_obj, APTracer::Entities::Ray_t& ray) -> void {
    Vec3f tangent;
    ...
    const double roughness = 1.0 - roughness_->get(tuv)[0];

    ray.colour_ += ray.mask_ * emission_;
    ray.mask_ *= texture_->get(tuv) * pow(newdir.dot(normal), roughness);
}
