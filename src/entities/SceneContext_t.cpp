...
auto APTracer::Entities::SceneContext_t::readXML(const std::string& filename) -> void {
    ...
    const std::string new_filename = next_filename(folder + scene_name_ + ".png");
    ...
    if (xml_transform_matrices != nullptr) {
        for (tinyxml2::XMLElement* xml_transform_matrix = xml_transform_matrices->FirstChildElement("transform_matrix"); xml_transform_matrix != nullptr;
             xml_transform_matrix                       = xml_transform_matrix->NextSiblingElement("transform_matrix")) {
            ++n_transform_matrices;
        }
    }

    if (xml_textures != nullptr) {
        for (tinyxml2::XMLElement* xml_texture = xml_textures->FirstChildElement("texture"); xml_texture != nullptr; xml_texture = xml_texture->NextSiblingElement("texture")) {
            ++n_textures;
        }
    }

    if (xml_mediums != nullptr) {
        for (tinyxml2::XMLElement* xml_medium = xml_mediums->FirstChildElement("medium"); xml_medium != nullptr; xml_medium = xml_medium->NextSiblingElement("medium")) {
            ++n_mediums;
    ...
    if (xml_materials != nullptr) {
        for (tinyxml2::XMLElement* xml_material = xml_materials->FirstChildElement("material"); xml_material != nullptr; xml_material = xml_material->NextSiblingElement("material")) {
            ++n_materials;
    ...
    if (xml_mesh_geometries != nullptr) {
        for (tinyxml2::XMLElement* xml_mesh_geometry = xml_mesh_geometries->FirstChildElement("mesh_geometry"); xml_mesh_geometry != nullptr;
             xml_mesh_geometry                       = xml_mesh_geometry->NextSiblingElement("mesh_geometry")) {
            ++n_mesh_geometries;
        }
    }

    if (xml_objects != nullptr) {
        for (tinyxml2::XMLElement* xml_object = xml_objects->FirstChildElement("object"); xml_object != nullptr; xml_object = xml_object->NextSiblingElement("object")) {
            ++n_objects;
    ...
    if (xml_directional_lights != nullptr) {
        for (tinyxml2::XMLElement* xml_directional_light = xml_directional_lights->FirstChildElement("directional_light"); xml_directional_light != nullptr;
             xml_directional_light                       = xml_directional_light->NextSiblingElement("directional_light")) {
    ...
    if (xml_skyboxes != nullptr) {
        for (tinyxml2::XMLElement* xml_skybox = xml_skyboxes->FirstChildElement("skybox"); xml_skybox != nullptr; xml_skybox = xml_skybox->NextSiblingElement("skybox")) {
            ++n_skyboxes;
    ...
    if (xml_imgbuffers != nullptr) {
        for (tinyxml2::XMLElement* xml_imgbuffer = xml_imgbuffers->FirstChildElement("imgbuffer"); xml_imgbuffer != nullptr; xml_imgbuffer = xml_imgbuffer->NextSiblingElement("imgbuffer")) {
            ++n_imgbuffers;
        }
    }

    if (xml_cameras != nullptr) {
        for (tinyxml2::XMLElement* xml_camera = xml_cameras->FirstChildElement("camera"); xml_camera != nullptr; xml_camera = xml_camera->NextSiblingElement("camera")) {
            ++n_cameras;
    ...
    std::vector<std::unique_ptr<std::tuple<std::unique_ptr<std::list<size_t>>, std::unique_ptr<std::list<std::string>>>>> materials_aggregate_list(n_materials);
    ...
    if (xml_materials != nullptr) {
        for (tinyxml2::XMLElement* xml_material = xml_materials->FirstChildElement("material"); xml_material != nullptr; xml_material = xml_material->NextSiblingElement("material")) {
            materials_[index_materials_] = create_material(xml_material,
                                                           ...
                                                           material_mix_list,
                                                           materials_aggregate_list[index_materials_],
                                                           material_transformations,
                                                           xml_textures,
                                                           xml_transform_matrices,
                                                           xml_materials,
                                                           xml_mediums);
            ++index_materials_;
        }
    ...
    for (size_t i = 0; i < materials_.size(); i++) {
        if (materials_aggregate_list[i]) {
            const size_t n = std::get<0>(*materials_aggregate_list[i])->size();
    ...
    if (xml_transform_matrices != nullptr) {
        ...
        for (tinyxml2::XMLElement* xml_transform_matrix = xml_transform_matrices->FirstChildElement("transform_matrix"); xml_transform_matrix != nullptr;
            ...
            if (transformations_pre != nullptr) {
                for (tinyxml2::XMLElement* transformation_pre = transformations_pre->FirstChildElement("transformation_pre"); transformation_pre != nullptr;
                     transformation_pre                       = transformation_pre->NextSiblingElement("transformation_pre")) {
    ...
    if (!material_transformations.transformations_pre.empty()) {
        for (const auto& transformations_pre: material_transformations.transformations_pre) {
            ...
            for (const tinyxml2::XMLElement* transformation_pre = transformations_pre.second->FirstChildElement("transformation_pre"); transformation_pre != nullptr;
                 transformation_pre                             = transformation_pre->NextSiblingElement("transformation_pre")) {
    ...
    if (xml_transform_matrices != nullptr) {
        ...
        for (tinyxml2::XMLElement* xml_transform_matrix = xml_transform_matrices->FirstChildElement("transform_matrix"); xml_transform_matrix != nullptr;
            ...
            if (transformations_post != nullptr) {
                for (tinyxml2::XMLElement* transformation_post = transformations_post->FirstChildElement("transformation_post"); transformation_post != nullptr;
                     transformation_post                       = transformation_post->NextSiblingElement("transformation_post")) {
    ...
    if (!material_transformations.transformations_post.empty()) {
        for (const auto& transformations_post: material_transformations.transformations_post) {
            ...
            for (const tinyxml2::XMLElement* transformation_post = transformations_post.second->FirstChildElement("transformation_post"); transformation_post != nullptr;
                 transformation_post                             = transformation_post->NextSiblingElement("transformation_post")) {
    ...
    std::cout << "Acceleration structure built in " << std::chrono::duration<double>(t_end - t_start).count() << "s." << std::endl << std::endl;
    ...
    if (xml_cameras != nullptr) {
        ...
        for (tinyxml2::XMLElement* xml_camera = xml_cameras->FirstChildElement("camera"); xml_camera != nullptr; xml_camera = xml_camera->NextSiblingElement("camera")) {
            ...
            if (render_mode == "accumulation") {
                ...
                std::cerr << "Warning: Render mode '" << render_mode << "', used by camera #" << index
                          << ", is unknown. Only 'accumulation', 'accumulation_write', 'single', and 'motion' exist for now. Ignoring." << std::endl;
...
auto APTracer::Entities::SceneContext_t::render() -> void {
    ...
    if (use_gl_) {
        ...
        if (render_mode == "accumulation") {
            ...
            std::cerr << "Warning: Render mode '" << render_mode << "', used by camera #" << 0
                      << ", is unknown. Only 'accumulation', 'accumulation_write', 'single', and 'motion' exist for now. Ignoring." << std::endl;
        }
    }
    else {
        for (size_t i = 0; i < cameras_.size(); i++) {
            ...
            if (render_mode == "accumulation") {
                ...
                std::cerr << "Warning: Render mode '" << render_mode << "', used by camera #" << i
                          << ", is unknown. Only 'accumulation', 'accumulation_write', 'single', and 'motion' exist for now. Ignoring." << std::endl;
            }
        }
    }
...
auto APTracer::Entities::SceneContext_t::create_medium(const tinyxml2::XMLElement* xml_medium,
    ...
    if (type == "absorber") {
        ...
        return std::make_unique<APTracer::Materials::Absorber_t>(APTracer::get_colour(xml_medium->Attribute("emission")),
                                                                 APTracer::get_colour(xml_medium->Attribute("colour")),
                                                                 xml_medium->DoubleAttribute("emission_distance"),
                                                                 xml_medium->DoubleAttribute("absorption_distance"),
                                                                 xml_medium->DoubleAttribute("ind"),
    ...
    if (type == "portal_scatterer") {
        ...
        require_attributes(xml_medium, attributes);
        std::unique_ptr<APTracer::Materials::PortalScatterer_t> medium(
            std::make_unique<APTracer::Materials::PortalScatterer_t>(get_transform_matrix(xml_medium->Attribute("transform_matrix"), xml_transform_matrices),
                                                                     std::list<Medium_t*>(),
    ...
    if (type == "scatterer_exp") {
        ...
        require_attributes(xml_medium, attributes);
        return std::make_unique<APTracer::Materials::ScattererExp_t>(APTracer::get_colour(xml_medium->Attribute("emission")),
                                                                     APTracer::get_colour(xml_medium->Attribute("colour")),
                                                                     xml_medium->DoubleAttribute("emission_distance"),
                                                                     xml_medium->DoubleAttribute("absorption_distance"),
                                                                     xml_medium->DoubleAttribute("scattering_distance"),
                                                                     xml_medium->DoubleAttribute("order"),
    ...
    if (type == "scatterer_exp_full") {
        const std::vector<const char*> attributes = {
            "emission", "colour", "scattering_emission", "scattering_colour", "emission_distance", "absorption_distance", "scattering_distance", "order", "scattering_angle", "ind", "priority"};
        require_attributes(xml_medium, attributes);
        return std::make_unique<APTracer::Materials::ScattererExpFull_t>(APTracer::get_colour(xml_medium->Attribute("emission")),
                                                                         APTracer::get_colour(xml_medium->Attribute("colour")),
                                                                         APTracer::get_colour(xml_medium->Attribute("scattering_emission")),
                                                                         APTracer::get_colour(xml_medium->Attribute("scattering_colour")),
                                                                         xml_medium->DoubleAttribute("emission_distance"),
                                                                         xml_medium->DoubleAttribute("absorption_distance"),
                                                                         xml_medium->DoubleAttribute("scattering_distance"),
                                                                         xml_medium->DoubleAttribute("order"),
    ...
    if (type == "scatterer") {
        ...
        require_attributes(xml_medium, attributes);
        return std::make_unique<APTracer::Materials::Scatterer_t>(APTracer::get_colour(xml_medium->Attribute("emission")),
                                                                  APTracer::get_colour(xml_medium->Attribute("colour")),
                                                                  xml_medium->DoubleAttribute("emission_distance"),
                                                                  xml_medium->DoubleAttribute("absorption_distance"),
                                                                  xml_medium->DoubleAttribute("scattering_distance"),
    ...
    if (type == "scatterer_full") {
        const std::vector<const char*> attributes = {
            "emission", "colour", "scattering_emission", "scattering_colour", "emission_distance", "absorption_distance", "scattering_distance", "ind", "priority"};
        require_attributes(xml_medium, attributes);
        return std::make_unique<APTracer::Materials::ScattererFull_t>(APTracer::get_colour(xml_medium->Attribute("emission")),
                                                                      APTracer::get_colour(xml_medium->Attribute("colour")),
                                                                      APTracer::get_colour(xml_medium->Attribute("scattering_emission")),
                                                                      APTracer::get_colour(xml_medium->Attribute("scattering_colour")),
                                                                      xml_medium->DoubleAttribute("emission_distance"),
    ...
    std::cerr << "Warning: Medium type '" << type << "' not implemented. Only 'absorber', 'nonabsorber', 'portal_scatterer', 'scatterer_exp', and 'scatterer' exists for now. Using 'nonabsorber'."
              << std::endl;
    return std::make_unique<APTracer::Materials::NonAbsorber_t>(1.0, 0);
}

auto APTracer::Entities::SceneContext_t::create_material(const tinyxml2::XMLElement* xml_material,
                                                         std::list<std::pair<APTracer::Materials::Portal_t*, std::list<size_t>>>& materials_medium_list,
    ...
    if (type == "diffuse") {
        ...
        return std::make_unique<APTracer::Materials::Diffuse_t>(
            APTracer::get_colour(xml_material->Attribute("emission")), APTracer::get_colour(xml_material->Attribute("colour")), xml_material->DoubleAttribute("roughness"));
    }
    if (type == "diffuse_normal") {
        const std::vector<const char*> attributes = {"emission", "colour", "normal_map", "roughness"};
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::DiffuseNormal_t>(APTracer::get_colour(xml_material->Attribute("emission")),
    ...
    if (type == "diffuse_full") {
        ...
        return std::make_unique<APTracer::Materials::DiffuseFull_t>(
            get_texture(xml_material->Attribute("emission_map"), xml_textures), get_texture(xml_material->Attribute("texture"), xml_textures), xml_material->DoubleAttribute("roughness"));
    }
    if (type == "diffuse_tex") {
        const std::vector<const char*> attributes = {"emission", "texture", "roughness"};
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::DiffuseTex_t>(
            APTracer::get_colour(xml_material->Attribute("emission")), get_texture(xml_material->Attribute("texture"), xml_textures), xml_material->DoubleAttribute("roughness"));
    }
    if (type == "diffuse_tex_normal") {
        const std::vector<const char*> attributes = {"emission", "texture", "normal_map", "roughness"};
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::DiffuseTexNormal_t>(APTracer::get_colour(xml_material->Attribute("emission")),
    ...
    if (type == "fresnelmix") {
        ...
        require_attributes(xml_material, attributes);
        std::unique_ptr<APTracer::Materials::FresnelMix_t> material(std::make_unique<APTracer::Materials::FresnelMix_t>(nullptr, nullptr, xml_material->DoubleAttribute("ind")));
    ...
    if (type == "fresnelmix_normal") {
        ...
        material_mix_lists.fresnel_mix_normal.emplace_back(material.get(),
                                                           get_material_mix(xml_material->Attribute("material_refracted"), xml_material->Attribute("material_reflected"), xml_materials));
        return std::move(material);
    }
    if (type == "fresnelmix_in") {
        const std::vector<const char*> attributes = {"material_refracted", "material_reflected", "ind"};
        require_attributes(xml_material, attributes);
        std::unique_ptr<APTracer::Materials::FresnelMixIn_t> material(std::make_unique<APTracer::Materials::FresnelMixIn_t>(nullptr, nullptr, xml_material->DoubleAttribute("ind")));
    ...
    if (type == "randommix") {
        ...
        require_attributes(xml_material, attributes);
        std::unique_ptr<APTracer::Materials::RandomMix_t> material(std::make_unique<APTracer::Materials::RandomMix_t>(nullptr, nullptr, xml_material->DoubleAttribute("ratio")));
    ...
    if (type == "randommix_in") {
        const std::vector<const char*> attributes = {"first_material", "second_material", "ratio"};
        require_attributes(xml_material, attributes);
        std::unique_ptr<APTracer::Materials::RandomMixIn_t> material(std::make_unique<APTracer::Materials::RandomMixIn_t>(nullptr, nullptr, xml_material->DoubleAttribute("ratio")));
    ...
    if (type == "reflective") {
        ...
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::Reflective_t>(APTracer::get_colour(xml_material->Attribute("emission")), APTracer::get_colour(xml_material->Attribute("colour")));
    }
    ...
    if (type == "reflective_fuzz") {
        ...
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::ReflectiveFuzz_t>(APTracer::get_colour(xml_material->Attribute("emission")),
                                                                       APTracer::get_colour(xml_material->Attribute("colour")),
                                                                       xml_material->DoubleAttribute("order"),
                                                                       xml_material->DoubleAttribute("diffusivity"));
    }
    if (type == "reflective_fuzz_normal") {
        const std::vector<const char*> attributes = {"emission", "colour", "normal_map", "order", "diffusivity"};
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::ReflectiveFuzzNormal_t>(APTracer::get_colour(xml_material->Attribute("emission")),
                                                                             APTracer::get_colour(xml_material->Attribute("colour")),
                                                                             get_texture(xml_material->Attribute("normal_map"), xml_textures),
                                                                             xml_material->DoubleAttribute("order"),
                                                                             xml_material->DoubleAttribute("diffusivity"));
    ...
    if (type == "reflective_fuzz_tex") {
        ...
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::ReflectiveFuzzTex_t>(APTracer::get_colour(xml_material->Attribute("emission")),
                                                                          get_texture(xml_material->Attribute("texture"), xml_textures),
                                                                          xml_material->DoubleAttribute("order"),
                                                                          xml_material->DoubleAttribute("diffusivity"));
    }
    if (type == "reflective_fuzz_tex_normal") {
        ...
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::ReflectiveFuzzTexNormal_t>(APTracer::get_colour(xml_material->Attribute("emission")),
                                                                                get_texture(xml_material->Attribute("texture"), xml_textures),
                                                                                get_texture(xml_material->Attribute("normal_map"), xml_textures),
                                                                                xml_material->DoubleAttribute("order"),
                                                                                xml_material->DoubleAttribute("diffusivity"));
    ...
    if (type == "reflective_refractive_normal") {
        ...
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::ReflectiveRefractiveNormal_t>(APTracer::get_colour(xml_material->Attribute("emission")),
                                                                                   APTracer::get_colour(xml_material->Attribute("colour")),
                                                                                   get_texture(xml_material->Attribute("normal_map"), xml_textures),
    ...
    if (type == "reflective_refractive_fuzz") {
        ...
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::ReflectiveRefractiveFuzz_t>(APTracer::get_colour(xml_material->Attribute("emission")),
                                                                                 APTracer::get_colour(xml_material->Attribute("colour")),
                                                                                 xml_material->DoubleAttribute("order"),
                                                                                 xml_material->DoubleAttribute("diffusivity"),
                                                                                 get_medium(xml_material->Attribute("medium"), xml_mediums));
    ...
    if (type == "refractive_fuzz") {
        const std::vector<const char*> attributes = {"emission", "colour", "medium", "order", "diffusivity"};
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::RefractiveFuzz_t>(APTracer::get_colour(xml_material->Attribute("emission")),
                                                                       APTracer::get_colour(xml_material->Attribute("colour")),
                                                                       xml_material->DoubleAttribute("order"),
                                                                       xml_material->DoubleAttribute("diffusivity"),
                                                                       get_medium(xml_material->Attribute("medium"), xml_mediums));
    ...
    if (type == "transparent") {
        ...
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::Transparent_t>(get_medium(xml_material->Attribute("medium"), xml_mediums));
    }
    if (type == "bounce_material") {
        const std::vector<const char*> attributes = {"max_bounces"};
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::BounceMaterial_t>(xml_material->UnsignedAttribute("max_bounces"));
    }
    if (type == "distance_material") {
        const std::vector<const char*> attributes = {"focus_distance", "exponent"};
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::DistanceMaterial_t>(xml_material->DoubleAttribute("focus_distance"), xml_material->DoubleAttribute("exponent"));
    }
    if (type == "texturemix") {
    ...
    if (type == "toon") {
        ...
        require_attributes(xml_material, attributes);
        return std::make_unique<APTracer::Materials::Toon_t>(APTracer::get_colour(xml_material->Attribute("colour")));
    }
    if (type == "aggregate") {
...
auto APTracer::Entities::SceneContext_t::create_directional_light(const tinyxml2::XMLElement* xml_directional_light, const tinyxml2::XMLElement* xml_transform_matrices)
    -> std::unique_ptr<DirectionalLight_t> {
    std::string type;
    ...
    if (type == "directional_light") {
        ...
        return std::make_unique<DirectionalLight_t>(APTracer::get_colour(xml_directional_light->Attribute("colour")),
...
auto APTracer::Entities::SceneContext_t::create_skybox(const tinyxml2::XMLElement* xml_skybox,
    ...
    if (type == "skybox_flat") {
        ...
        return std::make_unique<APTracer::Skyboxes::SkyboxFlat_t>(APTracer::get_colour(xml_skybox->Attribute("colour")));
    }
    ...
    if (type == "skybox_texture") {
        ...
        require_attributes(xml_skybox, attributes);
        return std::make_unique<APTracer::Skyboxes::SkyboxTexture_t>(get_texture(xml_skybox->Attribute("texture"), xml_textures));
    }
    if (type == "skybox_texture_sun") {
        const std::vector<const char*> attributes = {"texture", "light_position", "light_colour", "light_radius"};
        require_attributes(xml_skybox, attributes);
        return std::make_unique<APTracer::Skyboxes::SkyboxTextureSun_t>(get_texture(xml_skybox->Attribute("texture"), xml_textures),
                                                                        get_xy<double>(xml_skybox->Attribute("light_position")),
                                                                        APTracer::get_colour(xml_skybox->Attribute("light_colour")),
                                                                        xml_skybox->DoubleAttribute("light_radius"));
    }
    if (type == "skybox_texture_transformation") {
        const std::vector<const char*> attributes = {"texture", "transform_matrix"};
        require_attributes(xml_skybox, attributes);
        return std::make_unique<APTracer::Skyboxes::SkyboxTextureTransformation_t>(get_texture(xml_skybox->Attribute("texture"), xml_textures),
                                                                                   get_transform_matrix(xml_skybox->Attribute("transform_matrix"), xml_transform_matrices));
    }
    if (type == "skybox_texture_transformation_sun") {
        ...
        require_attributes(xml_skybox, attributes);
        return std::make_unique<APTracer::Skyboxes::SkyboxTextureTransformationSun_t>(get_texture(xml_skybox->Attribute("texture"), xml_textures),
                                                                                      get_transform_matrix(xml_skybox->Attribute("transform_matrix"), xml_transform_matrices),
                                                                                      get_xy<double>(xml_skybox->Attribute("light_position")),
                                                                                      APTracer::get_colour(xml_skybox->Attribute("light_colour")),
                                                                                      xml_skybox->DoubleAttribute("light_radius"));
...
auto APTracer::Entities::SceneContext_t::create_imgbuffer(const tinyxml2::XMLElement* xml_imgbuffer) -> std::unique_ptr<ImgBuffer_t> {
    ...
    if (type == "imgbuffer") {
        ...
        return std::make_unique<ImgBuffer_t>(xml_imgbuffer->UnsignedAttribute("resx"), xml_imgbuffer->UnsignedAttribute("resy"));
    }
...
auto APTracer::Entities::SceneContext_t::create_camera(const tinyxml2::XMLElement* xml_camera,
    ...
    if (type == "cam") {
        ...
        return std::make_unique<APTracer::Cameras::Cam_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                          filename,
                                                          APTracer::get_colour(xml_camera->Attribute("up")),
                                                          get_xy<double>(xml_camera->Attribute("fov")),
                                                          get_xy<unsigned int>(xml_camera->Attribute("subpix")),
                                                          get_imgbuffer(xml_camera->Attribute("imgbuffer"), xml_imgbuffers),
                                                          get_medium_list(xml_camera->Attribute("medium_list"), xml_mediums),
                                                          get_skybox(xml_camera->Attribute("skybox"), xml_skyboxes),
                                                          xml_camera->UnsignedAttribute("max_bounces"),
    ...
    if (type == "cam_aperture") {
        ...
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::CamAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                  filename,
                                                                  APTracer::get_colour(xml_camera->Attribute("up")),
                                                                  get_xy<double>(xml_camera->Attribute("fov")),
                                                                  get_xy<unsigned int>(xml_camera->Attribute("subpix")),
                                                                  ...
                                                                  get_skybox(xml_camera->Attribute("skybox"), xml_skyboxes),
                                                                  xml_camera->UnsignedAttribute("max_bounces"),
                                                                  xml_camera->DoubleAttribute("focus_distance"),
                                                                  xml_camera->DoubleAttribute("aperture"),
                                                                  xml_camera->DoubleAttribute("gammaind"));
    }
    if (type == "cam_motionblur") {
        ...
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::CamMotionblur_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                    filename,
                                                                    APTracer::get_colour(xml_camera->Attribute("up")),
                                                                    get_xy<double>(xml_camera->Attribute("fov")),
                                                                    get_xy<unsigned int>(xml_camera->Attribute("subpix")),
                                                                    ...
                                                                    get_skybox(xml_camera->Attribute("skybox"), xml_skyboxes),
                                                                    xml_camera->UnsignedAttribute("max_bounces"),
                                                                    get_xy<double>(xml_camera->Attribute("time")),
                                                                    xml_camera->DoubleAttribute("gammaind"));
    }
    if (type == "cam_motionblur_aperture") {
        ...
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::CamMotionblurAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                            filename,
                                                                            APTracer::get_colour(xml_camera->Attribute("up")),
                                                                            get_xy<double>(xml_camera->Attribute("fov")),
                                                                            get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "fishcam") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::FishCam_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                              filename,
                                                              APTracer::get_colour(xml_camera->Attribute("up")),
                                                              get_xy<double>(xml_camera->Attribute("fov")),
                                                              get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "fishcam_aperture") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "focus_distance", "aperture"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::FishCamAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                      filename,
                                                                      APTracer::get_colour(xml_camera->Attribute("up")),
                                                                      get_xy<double>(xml_camera->Attribute("fov")),
                                                                      get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "fishcam_motionblur") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "time"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::FishCamMotionblur_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                        filename,
                                                                        APTracer::get_colour(xml_camera->Attribute("up")),
                                                                        get_xy<double>(xml_camera->Attribute("fov")),
                                                                        get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "fishcam_motionblur_aperture") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "focus_distance", "aperture", "time"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::FishCamMotionblurAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                                filename,
                                                                                APTracer::get_colour(xml_camera->Attribute("up")),
                                                                                get_xy<double>(xml_camera->Attribute("fov")),
                                                                                get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "isocam") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::IsoCam_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                             filename,
                                                             APTracer::get_colour(xml_camera->Attribute("up")),
                                                             get_xy<double>(xml_camera->Attribute("fov")),
                                                             get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "isocam_aperture") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "focus_distance", "aperture"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::IsoCamAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                     filename,
                                                                     APTracer::get_colour(xml_camera->Attribute("up")),
                                                                     get_xy<double>(xml_camera->Attribute("fov")),
                                                                     get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "isocam_motionblur") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "time"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::IsoCamMotionblur_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                       filename,
                                                                       APTracer::get_colour(xml_camera->Attribute("up")),
                                                                       get_xy<double>(xml_camera->Attribute("fov")),
                                                                       get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "isocam_motionblur_aperture") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "focus_distance", "aperture", "time"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::IsoCamMotionblurAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                               filename,
                                                                               APTracer::get_colour(xml_camera->Attribute("up")),
                                                                               get_xy<double>(xml_camera->Attribute("fov")),
                                                                               get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "reccam") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::RecCam_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                             filename,
                                                             APTracer::get_colour(xml_camera->Attribute("up")),
                                                             get_xy<double>(xml_camera->Attribute("fov")),
                                                             get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "reccam_aperture") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "focus_distance", "aperture"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::RecCamAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                     filename,
                                                                     APTracer::get_colour(xml_camera->Attribute("up")),
                                                                     get_xy<double>(xml_camera->Attribute("fov")),
                                                                     get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "reccam_motionblur") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "time"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::RecCamMotionblur_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                       filename,
                                                                       APTracer::get_colour(xml_camera->Attribute("up")),
                                                                       get_xy<double>(xml_camera->Attribute("fov")),
                                                                       get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "reccam_motionblur_aperture") {
        const std::vector<const char*> attributes = {"fov", "subpix", "transform_matrix", "up", "imgbuffer", "medium_list", "skybox", "max_bounces", "gammaind", "focus_distance", "aperture", "time"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::RecCamMotionblurAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                               filename,
                                                                               APTracer::get_colour(xml_camera->Attribute("up")),
                                                                               get_xy<double>(xml_camera->Attribute("fov")),
                                                                               get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "cam_3d") {
        const std::vector<const char*> attributes = {
            "fov", "subpix", "transform_matrix", "up", "imgbuffer", "imgbuffer_L", "imgbuffer_R", "eye_dist", "medium_list", "skybox", "max_bounces", "gammaind"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::Cam3D_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                            filename,
                                                            APTracer::get_colour(xml_camera->Attribute("up")),
                                                            get_xy<double>(xml_camera->Attribute("fov")),
                                                            get_xy<unsigned int>(xml_camera->Attribute("subpix")),
                                                            get_imgbuffer(xml_camera->Attribute("imgbuffer"), xml_imgbuffers),
                                                            get_imgbuffer(xml_camera->Attribute("imgbuffer_L"), xml_imgbuffers),
                                                            get_imgbuffer(xml_camera->Attribute("imgbuffer_R"), xml_imgbuffers),
                                                            xml_camera->DoubleAttribute("eye_dist"),
                                                            get_medium_list(xml_camera->Attribute("medium_list"), xml_mediums),
                                                            get_skybox(xml_camera->Attribute("skybox"), xml_skyboxes),
                                                            xml_camera->UnsignedAttribute("max_bounces"),
                                                            xml_camera->DoubleAttribute("focus_distance"),
    ...
    if (type == "cam_3d_aperture") {
        const std::vector<const char*> attributes = {
            "fov", "subpix", "transform_matrix", "up", "imgbuffer", "imgbuffer_L", "imgbuffer_R", "eye_dist", "medium_list", "skybox", "max_bounces", "gammaind", "focus_distance", "aperture"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::Cam3DAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                    filename,
                                                                    APTracer::get_colour(xml_camera->Attribute("up")),
                                                                    get_xy<double>(xml_camera->Attribute("fov")),
                                                                    get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "cam_3d_motionblur") {
        const std::vector<const char*> attributes = {
            "fov", "subpix", "transform_matrix", "up", "imgbuffer", "imgbuffer_L", "imgbuffer_R", "eye_dist", "medium_list", "skybox", "max_bounces", "gammaind", "time"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::Cam3DMotionblur_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                      filename,
                                                                      APTracer::get_colour(xml_camera->Attribute("up")),
                                                                      get_xy<double>(xml_camera->Attribute("fov")),
                                                                      get_xy<unsigned int>(xml_camera->Attribute("subpix")),
    ...
    if (type == "cam_3d_motionblur_aperture") {
        const std::vector<const char*> attributes = {
            "fov", "subpix", "transform_matrix", "up", "imgbuffer", "imgbuffer_L", "imgbuffer_R", "eye_dist", "medium_list", "skybox", "max_bounces", "gammaind", "focus_distance", "aperture", "time"};
        require_attributes(xml_camera, attributes);
        return std::make_unique<APTracer::Cameras::Cam3DMotionblurAperture_t>(get_transform_matrix(xml_camera->Attribute("transform_matrix"), xml_transform_matrices),
                                                                              filename,
                                                                              APTracer::get_colour(xml_camera->Attribute("up")),
                                                                              get_xy<double>(xml_camera->Attribute("fov")),
                                                                              get_xy<unsigned int>(xml_camera->Attribute("subpix")),
...
auto APTracer::Entities::SceneContext_t::get_transform_matrix(std::string transform_matrix, const tinyxml2::XMLElement* xml_transform_matrices) -> TransformMatrix_t* {
    ...
    if (xml_transform_matrices != nullptr) {
        size_t index = 0;
        for (const tinyxml2::XMLElement* xml_transform_matrix = xml_transform_matrices->FirstChildElement("transform_matrix"); xml_transform_matrix != nullptr;
             xml_transform_matrix                             = xml_transform_matrix->NextSiblingElement("transform_matrix")) {
    ...
    while ((pos = string_material_list.find(delimiter)) != std::string::npos) {
        ...
        if (is_number(token)) {
            ...
            if (xml_materials != nullptr) {
                ...
                size_t index = 0;
                for (const tinyxml2::XMLElement* xml_material = xml_materials->FirstChildElement("material"); xml_material != nullptr; xml_material = xml_material->NextSiblingElement("material")) {
    ...
    while ((pos = string_medium_list.find(delimiter)) != std::string::npos) {
        ...
        if (is_number(token)) {
            ...
            if (xml_mediums != nullptr) {
                ...
                std::transform(token.begin(), token.end(), token.begin(), ::tolower);
                size_t index = 0;
                for (const tinyxml2::XMLElement* xml_medium = xml_mediums->FirstChildElement("medium"); xml_medium != nullptr; xml_medium = xml_medium->NextSiblingElement("medium")) {
...
auto APTracer::Entities::SceneContext_t::get_texture(std::string texture, const tinyxml2::XMLElement* xml_textures) const -> Texture_t* {
    ...
    if (xml_textures != nullptr) {
        ...
        size_t index = 0;
        for (const tinyxml2::XMLElement* xml_texture = xml_textures->FirstChildElement("texture"); xml_texture != nullptr; xml_texture = xml_texture->NextSiblingElement("texture")) {
...
auto APTracer::Entities::SceneContext_t::get_mesh_geometry(std::string mesh_geometry, const tinyxml2::XMLElement* xml_mesh_geometries) const -> MeshGeometry_t* {
    ...
    if (xml_mesh_geometries != nullptr) {
        ...
        size_t index = 0;
        for (const tinyxml2::XMLElement* xml_mesh_geometry = xml_mesh_geometries->FirstChildElement("mesh_geometry"); xml_mesh_geometry != nullptr;
             xml_mesh_geometry                             = xml_mesh_geometry->NextSiblingElement("mesh_geometry")) {
    ...
    while ((pos = lights_string.find(delimiter)) != std::string::npos) {
        ...
        if (is_number(token)) {
            ...
            if (xml_directional_lights != nullptr) {
                ...
                std::transform(token.begin(), token.end(), token.begin(), ::tolower);
                size_t index = 0;
                for (const tinyxml2::XMLElement* xml_directional_light = xml_directional_lights->FirstChildElement("directional_light"); xml_directional_light != nullptr;
                     xml_directional_light                             = xml_directional_light->NextSiblingElement("directional_light")) {
...
auto APTracer::Entities::SceneContext_t::get_imgbuffer(std::string imgbuffer, const tinyxml2::XMLElement* xml_imgbuffers) const -> ImgBuffer_t* {
    ...
    if (xml_imgbuffers != nullptr) {
        ...
        size_t index = 0;
        for (const tinyxml2::XMLElement* xml_imgbuffer = xml_imgbuffers->FirstChildElement("imgbuffer"); xml_imgbuffer != nullptr; xml_imgbuffer = xml_imgbuffer->NextSiblingElement("imgbuffer")) {
...
auto APTracer::Entities::SceneContext_t::get_skybox(std::string skybox, const tinyxml2::XMLElement* xml_skyboxes) const -> Skybox_t* {
    ...
    if (xml_skyboxes != nullptr) {
        ...
        size_t index = 0;
        for (const tinyxml2::XMLElement* xml_skybox = xml_skyboxes->FirstChildElement("skybox"); xml_skybox != nullptr; xml_skybox = xml_skybox->NextSiblingElement("skybox")) {
    ...
    while ((pos = objects_string.find(delimiter)) != std::string::npos) {
        ...
        if (is_number(token)) {
            ...
            if (xml_objects != nullptr) {
                ...
                std::transform(token.begin(), token.end(), token.begin(), ::tolower);
                size_t index = 0;
                for (const tinyxml2::XMLElement* xml_object = xml_objects->FirstChildElement("object"); xml_object != nullptr; xml_object = xml_object->NextSiblingElement("object")) {
